...
const LightPillar: React.FC<LightPillarProps> = ({
    ...
    if (!webGLSupported) {
        return (
            <div className={`w-full h-full flex items-center justify-center bg-black/10 ${className}`} style={{ mixBlendMode }}>
                WebGL not supported
            </div>
        );
    }

    return <div ref={containerRef} className={`w-full h-full ${className}`} style={{ mixBlendMode }} />;
};
